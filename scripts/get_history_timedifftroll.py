...
row to represent the troll tweet that came before a reply. So for example for same troll id, if we have
 index 17, the troll history will be [17], for index  20, we will have [17,20],since index 17 troll tweet came before index
 20 same troll tweet.
"""
for i in keys_group:
    various_grps = all_groups[i]
    ...
    if len(various_grps) > 1:
        ...
        for user in range(len_users):
            ...
            count_troll_hist = len(ind_list)
            length_troll_history[index_users[user]] = count_troll_hist
            #print(length_troll_history)
        ...
        length_troll_history[ind] = 1


        user_history_dict[ind] = []
        length_user_history[ind] = 0
...
time_before_tweet = {}
for gp in keys_group:
    merged_list = []
    various_grps = all_groups[gp]
    #print(various_grps)
